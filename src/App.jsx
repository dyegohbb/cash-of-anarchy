  ...
  return <main className="shell dashboardShell">
    ...
    {loading === 'dashboard' && !data ? <section className="dashboardLoading">Lendo sua planilha…</section> : <>
      <section className="metricGrid">
        ...
        <article className={`metricCard balanceCard ${position.cashBalance < 0 ? 'negative' : ''}`}><span>DINHEIRO DISPONÍVEL</span><strong>{formatMoney(position.cashBalance)}</strong><small>Anterior: {formatMoney(position.cashPrevious)} · {position.cashMovement < 0 ? 'saiu' : 'entrou'} no mês: {formatMoney(position.cashMovement)}</small></article>
        <article className="metricCard"><span>DÍVIDA NOS CARTÕES</span><strong className={position.cardDebt > 0 ? 'negativeText' : 'positiveText'}>{formatMoney(position.cardDebt)}</strong><small>{position.cardCount} carteira(s) · posição líquida {position.netPosition < 0 ? 'negativa' : 'positiva'}: {formatMoney(position.netPosition)}</small></article>
      </section>
      ...
      <section className="dashboardSection"><div className="sectionTitle"><div><span>MACRO</span><h3>Horizonte financeiro</h3></div><p>Caixa acumulado e dívidas dos cartões por competência.</p></div>
        <div className="monthTimeline">{(data?.planejamento || []).length ? data.planejamento.map((month) => <article key={month.competence} className={month.netPosition < 0 ? 'monthNegative' : ''}><span>{formatCompetenceLabel(month.competence)}</span><strong className={month.cashBalance < 0 ? 'negativeText' : 'positiveText'}>{formatMoney(month.cashBalance)}</strong><small>Dinheiro disponível · cartões {formatMoney(month.cardDebt)}</small><div><i style={{ width: `${Math.min(100, month.cardDebt ? (month.cardDebt / Math.max(1, month.cashBalance)) * 100 : 0)}%` }} /></div><p className={month.netPosition < 0 ? 'negativeText' : 'positiveText'}>Posição líquida {formatMoney(month.netPosition)}</p></article>) : <div className="dashboardEmpty">Nenhum lançamento futuro encontrado.</div>}</div>
      </section>
