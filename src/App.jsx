...
function DashboardScreen({ user, onNavigate, onSettingsUpdate, onSignOut }) {
  ...
  return <main className={`shell dashboardShell ${valuesVisible ? '' : 'valuesMasked'}`}>
    <header className="brand dashboardBrand"><span className="brandMark" aria-hidden="true">C$</span><div><p className="eyebrow">CENTRAL FINANCEIRA</p><h1>Cash Of Anarchy</h1></div><div className="dashboardAccountActions"><button className="accountButton" onClick={onSignOut} aria-label={`Sair da conta ${user.email}`}><span>{user.email}</span><strong>Sair</strong></button><button type="button" className="privacyButton" aria-pressed={valuesVisible} aria-label={valuesVisible ? 'Ocultar todos os valores' : 'Exibir todos os valores'} title={valuesVisible ? 'Ocultar valores' : 'Exibir valores'} onClick={() => setValuesVisible((visible) => !visible)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="3"/>{!valuesVisible && <path className="privacySlash" d="M4 4 20 20"/>}</svg><span>{valuesVisible ? 'Valores visíveis' : 'Valores ocultos'}</span></button></div></header>
